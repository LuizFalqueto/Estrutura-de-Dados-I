...
AVL *findLargestElementAVL(AVL *avl)
{
    ...
    if (rightAVL(temp) == NULL){
        return temp;
    }else{
        findLargestElementAVL(rightAVL(temp));
    }
}
...
{
    if((avl->left == NULL && avl->right == NULL)|| avl->info == NULL)
    {
        //printf("\nValor nao existe.");
        return NULL;
    }
    ...
    if(inf->value < avl->info->value)
    {
        if(ptr=deleteAVL(avl->left,inf))
        {
            if(avl->balance >= 2)
            {
                if(heightAVL(avl->right->left) <= heightAVL(avl->right->right))
                {
                    RR_RotationAVL(&avl);
                }else{
                    RL_RotationAVL(&avl);
    ...
    if(inf->value > avl->info->value)
    {
        if(ptr=deleteAVL(avl->right,inf))
        {
            if(avl->balance >= 2)
            {
                if(heightAVL(avl->right->left) <= heightAVL(avl->right->right))
    ...
    if(inf->value == avl->info->value)
    {
        if(rightAVL(avl) == NULL || leftAVL(avl) == NULL) //NO tem 1 filho
        {
          AVL *sai = avl;

          if(rightAVL(avl)==NULL)
          {
                avl = sai->left;
          }
          else
        ...
        {
            AVL *tempNoh = findLargestElementAVL(leftAVL(avl));
            avl->info = tempNoh->info;

            ptr = deleteAVL(leftAVL(avl), avl->info);

            if (avl->balance >= 2)
            {
                if(heightAVL(avl->left->left) >= heightAVL(avl->left->right))
                {
                    LL_RotationAVL(&avl);
                }
                else
